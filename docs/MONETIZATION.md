...
| 档位 | 价格 | 给什么 | 边界 / 文案 |
|---|---|---|---|
| **免费** | ¥0 | 必须华为账号登录；当前方向 **Mac host → Pad receiver**；**多 Pad 并发不限**（保留现有 #51，不削减） | 当前体验，不需承诺 |
...
- **有线复杂环境**：多设备/坞站/DevEco 抢 hdc 会瞬断但不冲突端口；已知边界 = 有线远端口仍硬编码 `8888`
  （`WiredDiscovery.swift`），若 Pad 实际回退到 `8889` 则 Mac 有线连不上。**✅ 已修（2026-06-02，独立于 C）**：
  平板 loopback（有线）只绑首选端口、不静默回退，占用即 `ListenFailed` 并日志提示"关闭占用应用或改用无线"
  （`TcpServerTransport.start`）；Mac `HdcFportTunnel` 的 `hdc fport` 失败时自动重试 3 次换本地端口（TOCTOU）。

## 8. 决策（已拍板 2026-06-02）

1. **后端**：✅ **v0.3 设备本地**账号绑定（Pad 登录 + 本地缓存，无服务器）；v0.4 接 IAP 时再上最小后端（账号↔公钥↔license）。
2. **免费并发**：✅ **免费保留多 Pad 并发**（不削减 #51）；**¥66 只卖未来新能力**（Android / 对称角色 / 主从自定义），不靠收口现有能力来逼付费。
3. **先做**：✅ **v0.2 有线加固**（§7：有线不静默回退远端口 + `hdc fport` TOCTOU 重试）—— 已实现，独立于 C。

**仍待你的硬前置**：**华为开发者后台** —— 创建 App + 开通 Account Kit / IAP 并产出 product id，是 v0.3（登录）/ v0.4（付费）开工的前提。这一步只能你做；就绪后我接 Pad 侧 SDK + Mac 侧配对授权。
