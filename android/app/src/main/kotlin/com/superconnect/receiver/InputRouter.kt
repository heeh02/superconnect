...
class InputRouter(
    ...
    private val surfaceW: () -> Float,
    private val surfaceH: () -> Float,
) {
    ...
    private val gc = GestureController { m ->
        ...
        val w = surfaceW().coerceAtLeast(1f); val h = surfaceH().coerceAtLeast(1f)
        val nx = if (isScroll) 0f else (m.x / w).coerceIn(0f, 1f)
        val ny = if (isScroll) 0f else (m.y / h).coerceIn(0f, 1f)
    ...
    /** Touch entry — the SurfaceView's OnTouchListener delegates here. Always consumes (returns true). */
    fun onTouch(ev: MotionEvent): Boolean {
        if (surfaceW() <= 0f || surfaceH() <= 0f) return true
    ...
    private fun emitPen(ev: MotionEvent, pi: Int, type: Int, buttons: Int) {
        ...
        sender.send(InputEvent(
            ...
            x = (ev.getX(pi) / w).coerceIn(0f, 1f), y = (ev.getY(pi) / h).coerceIn(0f, 1f),
